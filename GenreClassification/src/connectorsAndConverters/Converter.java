...
/*
 * 11/19/04 1.0 moved to LGPL. 12/12/99 Original verion. mdm@techie.com.
 * ----------------------------------------------------------------------- This
            ...
            try {
                for(; frame < frameCount; frame++) {
                    try {
                        ...
                        if(stop) { throw new JavaLayerException(ex.getLocalizedMessage(), ex); }
                    }
                }
            ...
            if(isDetail(VERBOSE_DETAIL)) {
                switch(updateID) {
                    case UPDATE_CONVERT_COMPLETE:
                        ...
                        pw.println();
                        pw.println("Converted " + param2 + " frames in " + param1 + " ms (" + (param1 / param2) + " ms per frame.)");
                }
            }
        }
