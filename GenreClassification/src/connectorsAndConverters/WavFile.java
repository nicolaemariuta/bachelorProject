...
public class WavFile
{
	...
	// Double to one channel
	// ------
	public int readFramesMono(double[] sampleBuffer, int numFramesToRead) throws IOException, WavFileException
	{
		return readFramesToMono(sampleBuffer, 0, numFramesToRead/numChannels);
	}
	
	
	public int readFramesToMono(double[] sampleBuffer, int offset, int numFramesToRead) throws IOException, WavFileException
	{
		if (ioState != IOState.READING) throw new IOException("Cannot read from WavFile instance");
		...
		for (int f=0 ; f<numFramesToRead ; f++)
		{
			...
			for (int c=0 ; c<numChannels ; c++)
			{
					...
					double sample =  floatOffset + (double) readSample() / floatScale;
	...
	public static void main(String[] args)
	{
		...
		try
		{
			String filename = "decoder\\TheBeatles-ComeTogether1969.wav";
			{
